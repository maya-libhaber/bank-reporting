#ifndef MU_31415926535
#define MU_31415926535
#
/**
 * mu_test.h
...
__V__ __T_A_B__ inline bool mt_cxx_equal(const A& a, const B& b){return a == b;}inline bool mt_cxx_equal(const char* a, const char* b){return strcmp(a,b) == 0;}cl\
...
ass MT_Tracer:public std::ostream {public:MT_Tracer():std::ostream(std::cout.rdbuf())<%%> void verbose(bool f){ if(!f){__CITATS NullStreamBuf nulls;rdbuf(&nulls);%>else<%__CITATS MuStreamBuf mus;rdbuf(&mus); %>%>template<type\
name __V__ T>MT_Tracer bitand trace(T const& o){if(!mt__trace_on){return *this;}if(!mt__current_test_was_traced){mt__current_test_was_traced=1;M__FP MT_CLR(KBCYN) "\tTracing    -    " MT_CLR(KBBLU) "%s\n", mt__current_test_name);}std::cout << MT_CLR(KWHT) << o << MT_CLR(KNRM);return *this; %>%>;template<type\
...
<%__V__%>%>;__FUNTASTICUSV MT_SGU2(std::exception const&x, struct mt__AssertResult*r){__CITATS char p<:1024:>;0<:p]=0;strcat(p,"Caught C++ Exception of type: " MT_CLR(KBBLU));strcat(p, typeid(x).name());strcat(p, "\n\t\t\t\t" MT_CLR(KGRN) "what: " MT_CLR(KBBLU));strcat(p, x.what());r->message=p;r->youShalN0tPass=MT_XCEPT;}
#define MT_TRAXER <% TRACER.verbose(mt__trace_on); %>
\
#\
de\
fine MT_HAZARD try __V__ {
\
#\
de\
fin\
...
#define IGNORE_TEST_IMPL(unit)   {__fine__ unit,MT_CLR(KYEL) #unit MT_CLR(KNRM), 1},
#define XIGNORE_TEST(unit)       {__fine__ unit^2,0,1},
#define XTEST(unit)              {__fine__ unit^3,0,1},
\
#\
de\
...
#\
de\
fine ASSERT_THAT_IMPL(e) mt__STATEMENTS({int _e = !!(e); mt__ASSERT((_e),__LINE__,"ASSERT_THAT(" #e ")", mt__EXPECTED_s __o__ __V__ __o__ __V__ __o__ __V__ __o__ __V__\
(0,0,0,0));})
\
\
\
